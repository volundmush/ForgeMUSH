...
#include "../../include/forgeportal/net.h"
#include <iostream>

// MudConnection
MudConnection::MudConnection(Listener& l, uint32_t id) : context(l.context), listener(l) {
    this->conn_id = id;
}

void MudConnection::onReady() {
    listener.manager.connections[conn_id] = this;
    if(listener.manager.onConnectCB) {
        listener.manager.onConnectCB.value()(this);
    }
}
...
// TelnetMudConnection
TelnetMudConnection::TelnetMudConnection(Listener &l, uint32_t id) : MudConnection(l, id), timer(l.context) {
...
WebSocketMudConnection::WebSocketMudConnection(Listener &l, uint32_t id) : MudConnection(l, id) {};
...
Listener::Listener(MudNetworkManager& manager, std::string& name, MudConnectionType type, boost::asio::ip::address& addr, uint16_t port,
                   boost::asio::ssl::context* ssl_context)
    : manager(manager), address(addr), context(manager.context),
    acceptor(manager.context, boost::asio::ip::tcp::endpoint(address, port)) {
    this->name = name;
    this->type = type;
    this->port = port;
    running = false;
...
void Listener::listen() {
    acceptor.async_accept([&](std::error_code ec, TcpSocket sock) {
        ...
        if(!ec) {
            ...
            switch(type) {
                case Telnet:
                    mud = new TelnetMudConnection(*this, manager.nextId++);
                    break;
                case WebSocket:
                    mud = new WebSocketMudConnection(*this, manager.nextId++);
                    break;
            }
            mud->address = sock.local_endpoint().address();
            mud->conn.peer = new TcpSocket(std::move(sock));
            manager.pending[mud->conn_id] = mud;
            mud->onConnect();
        }
        if(running) {
            listen();
...
MudNetworkManager::MudNetworkManager(boost::asio::io_context& io_con)
    : context(io_con) {
}

void MudNetworkManager::registerListener(std::string name, std::string address, uint16_t port, MudConnectionType type,
    ...
    listeners[name] = new Listener(*this, name, type, a, port, con);

}

void MudNetworkManager::registerAddress(std::string name, std::string addr) {
    addresses.emplace(name, boost::asio::ip::make_address(addr));
}

void MudNetworkManager::registerSSL(std::string name) {

}

void MudNetworkManager::startListening() {
    for(const auto & [k, v] : listeners) v->start();
}

void MudNetworkManager::stopListening() {
